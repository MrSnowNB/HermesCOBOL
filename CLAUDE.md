...
- **COBOL Tooling:** GnuCOBOL 3.2+ (`cobc -E -I ...` for preprocessing non-CICS programs); COBOL-REKT (smojol-cli Java JAR) for structural CFG/facts extraction
...
- **Key Constraints:** Raw-data-only policy (see README.md). No generated artifacts committed. CICS programs intentionally skip `cobc -E` (no translator provided in this repo; marked `preprocess_skipped=cics_no_translator`). All pipeline outputs go to gitignored dirs only: data/preprocessed/, data/rekt/, data/facts/, data/byte_layouts/, data/data_flow/, validation/, audit/
...
- **Domain vocabulary (use exactly):**
  ...
  - `Mode A` (preprocess hash roundtrip), `Mode B` (structural coverage via facts)
  - `facts/` (ground truth), `byte_layouts/`, `data_flow/` (extracted artifacts)
...
---
*Initial stub generated by repo-onboard on first context switch to /work/hermescobol. Review and edit as needed.*

...
Inputs:
  ...
  --annotations validation/pass1/<PROGRAM>_annotations.json (RF-07 read-only)
  --out    data/fallthrough/<PROGRAM>.json
...
def run_all() -> None:
    FALLTHROUGH_DIR.mkdir(parents=True, exist_ok=True)
    ann_dir = Path("validation") / "pass1"
    ...
    for cbl_path in unique:
        ...
        ann_path = ann_dir / f"{stem}_annotations.json"
        if not ann_path.exists():
            print(f"[skip] {stem} — no annotations found at {ann_path}")
            skipped += 1
            continue
        out_path = FALLTHROUGH_DIR / f"{stem}.json"
        try:
            result = run(cbl_path, ann_path, out_path)
            status = result["c5_assertion"]
            n_para = len(result["paragraphs"])
            implicit = sum(1 for p in result["paragraphs"] if p["terminator"] == "implicit")
            print(f"[ok] {stem}  paragraphs={n_para}  implicit={implicit}  c5={status}")
            completed += 1
        except Exception as exc:
            print(f"[ERROR] {stem}: {exc}")

    print(f"\n[ok] {completed}/{len(unique)} complete  ({skipped} skipped — no annotations)")
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--source", help="COBOL source file (read-only)")
    parser.add_argument(
        "--annotations",
        help="Pass 1 annotations JSON (validation/pass1/<PROG>_annotations.json)",
    )
    parser.add_argument("--out", help="Output JSON path")
    parser.add_argument(
        "--all",
        action="store_true",
        help="Process all programs with existing annotations",
    )
    args = parser.parse_args(argv)

    if args.all:
        if args.source or args.annotations or args.out:
            parser.error("--all is mutually exclusive with --source/--annotations/--out")
        run_all()
        return 0

    if not args.source or not args.annotations or not args.out:
        parser.error("--source, --annotations, and --out are required (or use --all)")
    ...
    if not annotations.exists():
        print(f"ERROR: annotations not found: {annotations}", file=sys.stderr)
        return 2

    result = run(source, annotations, out)

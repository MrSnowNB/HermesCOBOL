...
def _is_preprocess_available(prog: str, facts: dict) -> bool:
    """
    Determine if GnuCOBOL Mode A preprocessing output exists and is valid.

    Priority order:
    1. If facts.cics_present is True → preprocessing is never available (CICS
       programs cannot be preprocessed by cobc -E without a CICS translator).
    2. Otherwise check for the sentinel file produced by validate_roundtrip.py
       Mode A: validation/reconstructed/cbl/{prog}.pre.cbl
    3. Warn (but do not fail) if a .pre.cbl exists for a cics_present=True program
       (indicates an orphan file that should be cleaned up).
    """
    cics = facts.get("cics_present", False)
    pre_path = VALID_DIR / "reconstructed" / "cbl" / f"{prog}.pre.cbl"
    has_pre = pre_path.exists()

    if cics and has_pre:
        print(f"  [WARN] {prog}: stray .pre.cbl found for CICS program — "
              f"delete {pre_path} to resolve", flush=True)
...
def assemble_program(prog: str) -> dict[str, Any]:
    ...
    preprocess_available = _is_preprocess_available(prog, facts)
